...
export const pad: InstrumentFactory = () => {
  const reverb = new Tone.Freeverb({ roomSize: 0.85, dampening: 3000, wet: 0.4 }).toDestination()
  ...
  return {
    trigger: (time, pitch) => synth.triggerAttackRelease(pitch, '4n', time, 0.5),
    dispose: () => {
      synth.dispose()
      reverb.dispose()
    }
